document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadTable(search = '', sort = 'itemID', sortOrder = 'asc') {
        ...
        xhr.open('GET', 'summaryTable.php?search=' + encodeURIComponent(search) + '&sort=' + encodeURIComponent(sort) + '&order=' + encodeURIComponent(sortOrder), true);

        xhr.onload = function () {
            if (xhr.status === 200) {
                document.getElementById('summary-table').innerHTML = xhr.responseText;
